...
async def read_portfolio(portfolio_id: str, service: PortfolioService = Depends(get_portfolio_service),current_user: user.Login = Depends(oauth2.get_current_user),current_user_email: str = Depends(get_user_email)):
    # print("current user email is : " ,current_user_email)portfolio = await service.read_portfolio(portfolio_id)
    if portfolio is None:
        raise HTTPException(status_code=404, detail="Portfolio not found")
    return portfolio

@portfolio_router.put("/{portfolio_id}", response_model=PortfolioOut)
async def update_portfolio(portfolio_id: str, portfolio: PortfolioUpdate, service: PortfolioService = Depends(get_portfolio_service),current_user: user.Login = Depends(oauth2.get_current_user),current_user_email: str = Depends(get_user_email)):
...
async def delete_portfolio(portfolio_id: str, service: PortfolioService = Depends(get_portfolio_service),current_user: user.Login = Depends(oauth2.get_current_user),current_user_email: str = Depends(get_user_email)):
    # print("current user email is : " ,current_user_email)
...
async def list_portfolios(skip: int = 0, limit: int = 10, service: PortfolioService = Depends(get_portfolio_service),current_user: user.Login = Depends(oauth2.get_current_user),current_user_email: str = Depends(get_user_email)):
    # print("current user email is : " ,current_user_email)
...
async def get_portfolios_by_user(user_id: str, service: PortfolioService = Depends(get_portfolio_service),current_user: user.Login = Depends(oauth2.get_current_user),current_user_email: str = Depends(get_user_email)):
    # print("current user email is : " ,current_user_email)
